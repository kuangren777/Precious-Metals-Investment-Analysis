...
自动监控工商银行黄金价格，生成可视化图表，并在价格暴跌时发送邮件警告。支持AI智能分析投资建议。

## 功能特性

- ✨ **实时监控** - 每10分钟自动获取最新金价数据
- 📊 **多维度可视化** - 生成4张图表（实时、日线、周线、月线）
- 💾 **历史图表** - 保留带时间戳的历史图表，可配置保留期限
- ⚠️ **智能预警** - 自动检测价格暴跌（默认跌幅超过2%）
- 📧 **定时报告** - 每天下午3点和晚上12点发送分析报告
- 🤖 **AI分析** - 支持Claude AI智能分析，提供专业投资建议
- 🔧 **灵活配置** - 使用配置文件管理所有设置
...
- 保存位置：`charts/` 目录
- 文件命名：`gold_chart_YYYYMMDD_HHMMSS.png`
- 包含4个子图：
  1. **实时价格走势** - 最近的实时价格变化
  2. **日K线图** - 最近30天的价格波动区间
  3. **周度价格趋势** - 周度收盘价走势
